...
public class HelloTriangleSimple implements GLEventListener, KeyListener {
    ...
    private void setup() {
        try {
            ...
            log.error("Couldn't get OpenGL 4 profile", e);
            System.exit(1);
        }
    }
    ...
    private void initVertexArray(GL4 gl) {
        ...
        gl.glVertexArrayAttribBinding(vertexArrayName.get(0), Attr.POSITION, Stream.A);
        gl.glVertexArrayAttribBinding(vertexArrayName.get(0), Attr.COLOR, Stream.A);

        gl.glVertexArrayAttribFormat(vertexArrayName.get(0), Attr.POSITION, 2, GL_FLOAT, false, 0);
        gl.glVertexArrayAttribFormat(vertexArrayName.get(0), Attr.COLOR, 3, GL_FLOAT, false, 2 * 4);

        gl.glEnableVertexArrayAttrib(vertexArrayName.get(0), Attr.POSITION);
        gl.glEnableVertexArrayAttrib(vertexArrayName.get(0), Attr.COLOR);

        gl.glVertexArrayElementBuffer(vertexArrayName.get(0), bufferName.get(Buffer.ELEMENT));
        gl.glVertexArrayVertexBuffer(vertexArrayName.get(0), Stream.A, bufferName.get(Buffer.VERTEX), 0, (2 + 3) * 4);
    }
